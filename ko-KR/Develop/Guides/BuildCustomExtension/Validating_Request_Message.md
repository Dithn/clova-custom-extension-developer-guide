...
1. Clova의 서명용 RSA 공개 키를 아래 URI에서 다운로드하십시오.<br />
  ```
  {{ book.ServiceEnv.PublicKeyURIforCEKMessageValidation }}
  ```
2. [HTTP 헤더](/Develop/References/HTTP_Message.md#HTTPHeader)에서 `SignatureCEK` 필드의 값을 확보하십시오.<br />
...
3. 1 번 항목에서 다운로드한 RSA 공개 키와 2 번 항목에서 획득한 `SignatureCEK` 헤더 값을 이용하여 <a href="https://tools.ietf.org/html/rfc3447#section-5.2" target="_blank">검증(verify)</a>하십시오.
