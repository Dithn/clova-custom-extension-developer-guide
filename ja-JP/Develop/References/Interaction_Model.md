...
対話モデルの[カスタムインテントを登録する](/Develop/Register_Extension.md#AddCustomIntent)際にスロットを指定するときは、次の図のようにサンプル発話の **数字** の部分を選択します。  
...
対話モデルの[カスタムインテントを登録する](/Develop/Register_Extension.md#AddCustomIntent)際にスロットを指定するときは、次の図のようにサンプル発話の **単位** の部分を選択します。  
...
対話モデルの[カスタムインテントを登録する](/Develop/Register_Extension.md#AddCustomIntent)際にスロットを指定するときは、次の図のようにサンプル発話の **数字と単位** をまとめて選択します（[`CLOVA.NUMBER`](#ClovaNumber)のみ選択のやり方に例外があります）。  
...
対話モデルの[カスタムインテントを登録する](/Develop/Register_Extension.md#AddCustomIntent)際に、サンプル発話のどの部分をスロットとして処理するかをドラッグして選択する必要があります。`CLOVA.NUMBER`の場合は、このときに選択する範囲によって取得する値が異なります。
